...
// src/infrastructure/webhook/__tests__/quicknode-verifier.test.ts
...
var PAYLOAD = JSON.stringify({ event: "test", data: { id: 1 } });
function buildSignature(secret, nonce, timestamp, payload) {
  const hmac = createHmac2("sha256", secret);
  hmac.update(nonce + timestamp + payload);
...
describe("QuickNodeVerifier", () => {
  describe("verify", () => {
    it("returns true when the HMAC signature matches", () => {
      ...
      globalExpect(verifier.verify(PAYLOAD, signature, NONCE, TIMESTAMP)).toBe(true);
    });
    it("returns false when the signature is wrong", () => {
      const verifier = new QuickNodeVerifier(SECRET);
      globalExpect(verifier.verify(PAYLOAD, "wrongsignature", NONCE, TIMESTAMP)).toBe(false);
    });
    it("returns false when the payload is tampered", () => {
      const verifier = new QuickNodeVerifier(SECRET);
      const signature = buildSignature(SECRET, NONCE, TIMESTAMP, PAYLOAD);
      const tamperedPayload = JSON.stringify({ event: "test", data: { id: 2 } });
      globalExpect(verifier.verify(tamperedPayload, signature, NONCE, TIMESTAMP)).toBe(false);
    });
    it("returns false when the nonce differs", () => {
      const verifier = new QuickNodeVerifier(SECRET);
      const signature = buildSignature(SECRET, NONCE, TIMESTAMP, PAYLOAD);
      globalExpect(verifier.verify(PAYLOAD, signature, "differentnonce", TIMESTAMP)).toBe(false);
    });
    it("returns false when the timestamp differs", () => {
      const verifier = new QuickNodeVerifier(SECRET);
      const signature = buildSignature(SECRET, NONCE, TIMESTAMP, PAYLOAD);
      globalExpect(verifier.verify(PAYLOAD, signature, NONCE, "9999999999")).toBe(false);
    });
    it("returns false when the secret is wrong", () => {
      const verifier = new QuickNodeVerifier("wrong-secret");
      const signature = buildSignature(SECRET, NONCE, TIMESTAMP, PAYLOAD);
      globalExpect(verifier.verify(PAYLOAD, signature, NONCE, TIMESTAMP)).toBe(false);
    });
  });
});
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=quicknode-verifier.test.js.map