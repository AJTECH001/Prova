...
// src/infrastructure/auth/__tests__/jwt.service.test.ts
var TEST_PAYLOAD = {
  sub: "user-123",
...
describe("JwtService", () => {
  ...
  describe("generateTokenPair", () => {
    it("returns accessToken, refreshToken, and expiresIn", async () => {
      ...
      globalExpect(result).toHaveProperty("accessToken");
      globalExpect(result).toHaveProperty("refreshToken");
      globalExpect(result).toHaveProperty("expiresIn");
      globalExpect(typeof result.accessToken).toBe("string");
      globalExpect(typeof result.refreshToken).toBe("string");
      globalExpect(typeof result.expiresIn).toBe("number");
      globalExpect(result.expiresIn).toBeGreaterThan(0);
    });
    it("generates distinct access and refresh tokens", async () => {
      const result = await service.generateTokenPair(TEST_PAYLOAD);
      globalExpect(result.accessToken).not.toBe(result.refreshToken);
    });
  });
  describe("verifyAccessToken", () => {
    it("returns payload for a valid access token", async () => {
      ...
      globalExpect(payload.sub).toBe(TEST_PAYLOAD.sub);
      globalExpect(payload.walletAddress).toBe(TEST_PAYLOAD.walletAddress);
      globalExpect(payload.walletProvider).toBe(TEST_PAYLOAD.walletProvider);
      globalExpect(payload.email).toBe(TEST_PAYLOAD.email);
    });
    it("throws for an invalid token string", async () => {
      await globalExpect(service.verifyAccessToken("not.a.token")).rejects.toThrow();
    });
    it("throws for a structurally valid but tampered token", async () => {
      const { accessToken } = await service.generateTokenPair(TEST_PAYLOAD);
      const parts = accessToken.split(".");
      const tamperedToken = parts[0] + "." + parts[1] + ".invalidsignatureXXX";
      await globalExpect(service.verifyAccessToken(tamperedToken)).rejects.toThrow();
    });
  });
  describe("verifyRefreshToken", () => {
    it("returns sub for a valid refresh token", async () => {
      ...
      globalExpect(result.sub).toBe(TEST_PAYLOAD.sub);
    });
    it("throws for an invalid refresh token", async () => {
      await globalExpect(service.verifyRefreshToken("garbage")).rejects.toThrow();
    });
    it("throws when using an access token as a refresh token with wrong issuer", async () => {
      await globalExpect(service.verifyRefreshToken("eyJhbGciOiJIUzI1NiJ9.invalid")).rejects.toThrow();
    });
  });
});
