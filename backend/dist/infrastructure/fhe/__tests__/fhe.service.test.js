...
// src/infrastructure/fhe/__tests__/fhe.service.test.ts
process.env.JWT_SECRET = process.env.JWT_SECRET || "test-secret-for-fhe-service";
var OWNER_ADDRESS = "0x1234567890abcdef1234567890abcdef12345678";
var USER_ADDRESS = "0xabcdef1234567890abcdef1234567890abcdef12";
function makeBatchResponse(amountData, ownerData) {
  return {
    results: [
      {
        type: "euint64",
...
describe("FheService", () => {
  ...
  describe("encryptEscrowData", () => {
    it("returns an EncryptedEscrowData with encryptedAmount and encryptedOwner", async () => {
      ...
      globalExpect(result.encryptedAmount).toBeDefined();
      globalExpect(result.encryptedOwner).toBeDefined();
    });
    it("encrypted amount has euint64 type with correct data from worker", async () => {
      ...
      globalExpect(result.encryptedAmount.type).toBe("euint64");
      globalExpect(result.encryptedAmount.data).toBe("0xabc123");
      globalExpect(result.encryptedAmount.inputProof).toBe("0xaaaa");
      globalExpect(result.encryptedAmount.securityZone).toBe(0);
      globalExpect(result.encryptedAmount.utype).toBe(4);
    });
    it("encrypted owner has eaddress type with correct utype", async () => {
      ...
      const service = new FheService();
      const result = await service.encryptEscrowData(500n, OWNER_ADDRESS, USER_ADDRESS);
      globalExpect(result.encryptedOwner.type).toBe("eaddress");
      globalExpect(result.encryptedOwner.data).toBe("0xdef456");
      globalExpect(result.encryptedOwner.securityZone).toBe(0);
      globalExpect(result.encryptedOwner.utype).toBe(12);
    });
    it("stores plaintext amount and owner address", async () => {
      ...
      globalExpect(result.plaintextAmount).toBe(amount);
      globalExpect(result.plaintextOwner).toBe(OWNER_ADDRESS);
    });
    it("stores the user address", async () => {
      ...
      globalExpect(result.userAddress).toBe(USER_ADDRESS);
    });
    it("getContractCallParameters returns a tuple of 4 elements for each encrypted field", async () => {
      ...
      globalExpect(params.encrypted_amount).toHaveLength(4);
      globalExpect(params.encrypted_owner).toHaveLength(4);
      globalExpect(params.resolver).toBe("0x0000000000000000000000000000000000000000");
      globalExpect(params.resolver_data).toBe("0x");
    });
    it("sends correct payload to the FHE worker", async () => {
      ...
      const batchCall = fetchMock.mock.calls.find(
        (c) => typeof c[0] === "string" && c[0].includes("/api/v1/encrypt/batch")
      );
      globalExpect(batchCall).toBeDefined();
      const body = JSON.parse(batchCall[1].body);
      globalExpect(body.userAddress).toBe(USER_ADDRESS);
      globalExpect(body.items).toEqual([
    ...
    it("throws when the FHE worker returns an error", async () => {
      ...
      const service = new FheService();
      await globalExpect(service.encryptEscrowData(100n, OWNER_ADDRESS, USER_ADDRESS)).rejects.toThrow(
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=fhe.service.test.js.map