...
describe("ApplicationHttpError", () => {
  describe("constructor", () => {
    it("sets statusCode, message, and name", () => {
      ...
      globalExpect(error.message).toBe("bad input");
      globalExpect(error.name).toBe("ApplicationHttpError");
    });
    ...
    it("leaves details undefined when not provided", () => {
      const error = new ApplicationHttpError(400, "bad input");
      globalExpect(error.details).toBeUndefined();
    });
    it("is an instance of Error", () => {
      const error = new ApplicationHttpError(500, "error");
      globalExpect(error).toBeInstanceOf(Error);
    });
  });
  describe("badRequest", () => {
    it("returns 400 with given message", () => {
      ...
      globalExpect(error.statusCode).toBe(400);
      globalExpect(error.message).toBe("Invalid input");
      globalExpect(error.name).toBe("ApplicationHttpError");
      globalExpect(error.details).toBeUndefined();
    });
  });
  describe("unauthorized", () => {
    it("returns 401 with given message", () => {
      ...
      globalExpect(error.message).toBe("Not authenticated");
      globalExpect(error.name).toBe("ApplicationHttpError");
    });
  });
  describe("forbidden", () => {
    it("returns 403 with given message", () => {
      ...
      globalExpect(error.message).toBe("Access denied");
      globalExpect(error.name).toBe("ApplicationHttpError");
    });
  });
  describe("notFound", () => {
    it("returns 404 with given message", () => {
      ...
      globalExpect(error.message).toBe("Resource not found");
      globalExpect(error.name).toBe("ApplicationHttpError");
    });
  });
  describe("conflict", () => {
    it("returns 409 with given message", () => {
      ...
      globalExpect(error.message).toBe("Already exists");
      globalExpect(error.name).toBe("ApplicationHttpError");
    });
  });
  describe("validationError", () => {
    it("returns 422 with fixed message and provided details", () => {
      ...
      globalExpect(error.message).toBe("Validation failed");
      globalExpect(error.details).toEqual(details);
      globalExpect(error.name).toBe("ApplicationHttpError");
    });
    it("accepts any shape as details", () => {
      const error = ApplicationHttpError.validationError("simple string details");
      globalExpect(error.details).toBe("simple string details");
    });
  });
