...
describe("ETH_ADDRESS_REGEX", () => {
  it("matches a valid lowercase hex address", () => {
    globalExpect(ETH_ADDRESS_REGEX.test("0xabcdef1234567890abcdef1234567890abcdef12")).toBe(true);
  });
  ...
  it("rejects an address missing 0x prefix", () => {
    globalExpect(ETH_ADDRESS_REGEX.test("abcdef1234567890abcdef1234567890abcdef12")).toBe(false);
  });
  it("rejects an address that is too short", () => {
    globalExpect(ETH_ADDRESS_REGEX.test("0xabcdef")).toBe(false);
  });
  it("rejects an address that is too long", () => {
    globalExpect(ETH_ADDRESS_REGEX.test("0xabcdef1234567890abcdef1234567890abcdef1234")).toBe(false);
  });
  it("rejects an address with non-hex characters", () => {
    globalExpect(ETH_ADDRESS_REGEX.test("0xZZZZZZ1234567890abcdef1234567890abcdef12")).toBe(false);
  });
});
describe("TX_HASH_REGEX", () => {
  it("matches a valid lowercase 64-char hex hash", () => {
    globalExpect(TX_HASH_REGEX.test("0x" + "a".repeat(64))).toBe(true);
  });
  it("matches a valid uppercase 64-char hex hash", () => {
    globalExpect(TX_HASH_REGEX.test("0x" + "A".repeat(64))).toBe(true);
  });
  it("matches a valid mixed-case 64-char hex hash", () => {
    globalExpect(TX_HASH_REGEX.test("0x" + "aAbB".repeat(16))).toBe(true);
  });
  it("rejects a hash missing 0x prefix", () => {
    globalExpect(TX_HASH_REGEX.test("a".repeat(64))).toBe(false);
  });
  it("rejects a hash that is too short", () => {
    globalExpect(TX_HASH_REGEX.test("0x" + "a".repeat(63))).toBe(false);
  });
  it("rejects a hash that is too long", () => {
    globalExpect(TX_HASH_REGEX.test("0x" + "a".repeat(65))).toBe(false);
  });
  it("rejects a hash with non-hex characters", () => {
    globalExpect(TX_HASH_REGEX.test("0x" + "z".repeat(64))).toBe(false);
  });
});
describe("ethAddressSchema", () => {
  it("parses a valid Ethereum address", () => {
    const address = "0xabcdef1234567890abcdef1234567890abcdef12";
    globalExpect(ethAddressSchema.parse(address)).toBe(address);
  });
  it("throws for an invalid address", () => {
    globalExpect(() => ethAddressSchema.parse("not-an-address")).toThrow();
  });
  it("throws for an address without 0x prefix", () => {
    globalExpect(() => ethAddressSchema.parse("abcdef1234567890abcdef1234567890abcdef12")).toThrow();
  });
  it("throws for an empty string", () => {
    globalExpect(() => ethAddressSchema.parse("")).toThrow();
  });
});
describe("txHashSchema", () => {
  it("parses a valid transaction hash", () => {
    const hash = "0x" + "a".repeat(64);
    globalExpect(txHashSchema.parse(hash)).toBe(hash);
  });
  it("throws for a hash with wrong length", () => {
    globalExpect(() => txHashSchema.parse("0x" + "a".repeat(63))).toThrow();
  });
  it("throws for a hash without 0x prefix", () => {
    globalExpect(() => txHashSchema.parse("a".repeat(64))).toThrow();
  });
  it("throws for an empty string", () => {
    globalExpect(() => txHashSchema.parse("")).toThrow();
  });
});
describe("validate", () => {
  it("returns parsed data when schema matches", () => {
    const schema = z2.object({ name: z2.string(), age: z2.number() });
    const result = validate(schema, { name: "Alice", age: 30 });
    globalExpect(result).toEqual({ name: "Alice", age: 30 });
  });
  it("coerces and transforms data per schema rules", () => {
    const schema = z2.object({ value: z2.string().transform((v2) => v2.toUpperCase()) });
    const result = validate(schema, { value: "hello" });
    globalExpect(result).toEqual({ value: "HELLO" });
  });
  it("throws ZodError when data is invalid", () => {
    const schema = z2.object({ name: z2.string() });
    globalExpect(() => validate(schema, { name: 123 })).toThrow();
  });
  it("throws ZodError for missing required fields", () => {
    const schema = z2.object({ required: z2.string() });
    globalExpect(() => validate(schema, {})).toThrow();
  });
  it("works with primitive schemas", () => {
    globalExpect(validate(z2.string(), "hello")).toBe("hello");
    globalExpect(validate(z2.number(), 42)).toBe(42);
  });
});
