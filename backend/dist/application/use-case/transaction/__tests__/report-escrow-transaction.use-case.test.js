...
// src/application/use-case/transaction/__tests__/report-escrow-transaction.use-case.test.ts
...
// src/application/use-case/transaction/__tests__/report-escrow-transaction.use-case.test.ts
var USER_ID = randomUUID();
var TX_HASH = "0xtxhash123";
function makeEscrow(userId = USER_ID) {
  return new Escrow({
    id: randomUUID(),
    publicId: randomUUID(),
    userId,
...
describe("ReportEscrowTransactionUseCase", () => {
  ...
  it("marks escrow as PROCESSING and stores the tx_hash", async () => {
    ...
    globalExpect(result.tx_hash).toBe(TX_HASH);
    globalExpect(result.status).toBe("PROCESSING" /* PROCESSING */);
    globalExpect(result.entity_id).toBe(escrow.publicId);
  });
  it("reconciles with a buffered EscrowCreated event \u2014 escrow becomes ON_CHAIN", async () => {
    ...
    const result = await useCase.execute({ entity_id: escrow.publicId, tx_hash: TX_HASH }, USER_ID);
    globalExpect(result.status).toBe("ON_CHAIN" /* ON_CHAIN */);
    const updated = await escrowRepo.findByPublicId(escrow.publicId);
    globalExpect(updated.onChainEscrowId).toBe("99");
  });
  it("deletes the buffered event after reconciliation", async () => {
    ...
    globalExpect(await escrowEventRepo.findByTxHash(TX_HASH)).toBeNull();
  });
  ...
  it("throws 404 when escrow is not found", async () => {
    await globalExpect(useCase.execute({ entity_id: "nonexistent", tx_hash: TX_HASH }, USER_ID)).rejects.toMatchObject({
      statusCode: 404
    });
  });
  it("throws 403 when escrow belongs to a different user", async () => {
    ...
    await escrowRepo.save(escrow);
    await globalExpect(useCase.execute({ entity_id: escrow.publicId, tx_hash: TX_HASH }, USER_ID)).rejects.toMatchObject({
      statusCode: 403
    });
  });
  it("throws ApplicationHttpError for missing escrow", async () => {
    await globalExpect(useCase.execute({ entity_id: "missing", tx_hash: TX_HASH }, USER_ID)).rejects.toBeInstanceOf(
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=report-escrow-transaction.use-case.test.js.map