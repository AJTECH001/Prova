...
// src/application/use-case/withdrawal/__tests__/create-withdrawal.use-case.test.ts
...
function generatePublicId() {
  ...
  let result = "WD-";
  for (let i = 0; i < 8; i++) {
    result += chars4[Math.floor(Math.random() * chars4.length)];
  }
  return result;
}
...
var WALLET = "0xuser";
var RECIPIENT = "0xrecipient";
function makeSettledEscrow(onChainId, userId = USER_ID) {
  return new Escrow({
    id: randomUUID2(),
...
describe("CreateWithdrawalUseCase", () => {
  ...
  it("sums estimated amount from multiple escrows", async () => {
    ...
    globalExpect(result.estimated_amount).toBe(300);
  });
  ...
  it("rejects with 403 if escrow belongs to a different user", async () => {
    await escrowRepo.save(makeSettledEscrow("7", randomUUID2()));
    await globalExpect(
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=create-withdrawal.use-case.test.js.map