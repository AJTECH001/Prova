...
// src/application/use-case/escrow/__tests__/create-escrow.use-case.test.ts
...
// src/application/use-case/escrow/__tests__/create-escrow.use-case.test.ts
var USER_ID = randomUUID2();
var WALLET = "0xabc123walletaddress";
var ESCROW_CONTRACT = "0xescrowcontract";
function makeFakeEncryptedValue(type3) {
  return new EncryptedValue({
    type: type3,
    data: `0xfakedata_${type3}`,
    securityZone: 0,
    utype: type3 === "euint64" ? 4 : 12,
    inputProof: `0xfakeproof_${type3}`,
    userAddress: WALLET
  });
}
function makeFakeEncryptedEscrowData() {
  return new EncryptedEscrowData({
    ...
    userAddress: WALLET,
    plaintextAmount: BigInt(1e8),
    plaintextOwner: WALLET
  });
}
function makeMockFheService() {
  return {
    encryptEscrowData: async () => makeFakeEncryptedEscrowData()
  };
}
function makeCreateEscrowDto(overrides = {}) {
  return {
    type: "payment",
    amount: 100,
...
describe("CreateEscrowUseCase", () => {
  ...
  it("returns encrypted abi_parameters in server mode", async () => {
    ...
    globalExpect(result.abi_parameters).toHaveProperty("encrypted_owner");
    globalExpect(result.abi_parameters).toHaveProperty("encrypted_amount");
    globalExpect(result.abi_parameters).toHaveProperty("resolver");
    globalExpect(result.abi_parameters).toHaveProperty("resolver_data");
  });
  it("does not return owner_address or amount in server mode", async () => {
    const result = await useCase.execute(makeCreateEscrowDto(), USER_ID, WALLET, "server");
    globalExpect(result).not.toHaveProperty("owner_address");
    globalExpect(result).not.toHaveProperty("amount_smallest_unit");
  });
  it("returns plaintext owner_address and amount in client mode", async () => {
    ...
    globalExpect(result.owner_address).toBe(WALLET);
    globalExpect(result.amount).toBe(100);
  });
  it("does not call fheService in client mode", async () => {
    ...
    globalExpect(called).toBe(false);
  });
  it('converts 100 USDC to "100000000" in client mode', async () => {
    ...
    globalExpect(result.amount_smallest_unit).toBe("100000000");
  });
  it("converts fractional USDC amount correctly to smallest unit in client mode", async () => {
    ...
    globalExpect(result.amount_smallest_unit).toBe("1500000");
  });
  it("saves escrow to repository with PENDING status", async () => {
    ...
    globalExpect(saved).not.toBeNull();
    globalExpect(saved.status).toBe("PENDING" /* PENDING */);
  });
  it("saves escrow to repository with correct userId", async () => {
    const result = await useCase.execute(makeCreateEscrowDto(), USER_ID, WALLET);
    const saved = await escrowRepo.findByPublicId(result.public_id);
    globalExpect(saved.userId).toBe(USER_ID);
  });
  it("saves escrow with the provided amount and currency", async () => {
    ...
    const result = await useCase.execute(dto, USER_ID, WALLET);
    const saved = await escrowRepo.findByPublicId(result.public_id);
    globalExpect(saved.amount).toBe(250);
    globalExpect(saved.currency.code).toBe("USDC");
  });
  it("returns ESCROW_CONTRACT_ADDRESS as contract_address in server mode", async () => {
    const result = await useCase.execute(makeCreateEscrowDto(), USER_ID, WALLET, "server");
    globalExpect(result.contract_address).toBe(ESCROW_CONTRACT);
  });
  ...
  it("returns the correct abi_function_signature", async () => {
    const result = await useCase.execute(makeCreateEscrowDto(), USER_ID, WALLET);
    globalExpect(result.abi_function_signature).toBe(
  ...
  it("encrypted_owner and encrypted_amount are tuples of correct shape", async () => {
    ...
    globalExpect(Array.isArray(encrypted_owner)).toBe(true);
    globalExpect(encrypted_owner).toHaveLength(4);
    globalExpect(typeof encrypted_owner[0]).toBe("string");
    globalExpect(typeof encrypted_owner[1]).toBe("number");
    globalExpect(typeof encrypted_owner[2]).toBe("number");
    globalExpect(typeof encrypted_owner[3]).toBe("string");
    globalExpect(Array.isArray(encrypted_amount)).toBe(true);
    globalExpect(encrypted_amount).toHaveLength(4);
  });
  it("uses server mode by default when encryptionMode is omitted", async () => {
    const result = await useCase.execute(makeCreateEscrowDto(), USER_ID, WALLET);
    globalExpect(result).toHaveProperty("abi_parameters");
    globalExpect(result.abi_parameters).toHaveProperty("encrypted_owner");
  });
});
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=create-escrow.use-case.test.js.map