...
// src/application/use-case/webhook/__tests__/process-escrow-event.use-case.test.ts
...
// src/application/use-case/webhook/__tests__/process-escrow-event.use-case.test.ts
var TX_HASH = "0xabc123txhash";
var ESCROW_ID = "escrow-42";
function makeEscrow(status, txHash, onChainId) {
  return new Escrow({
    id: randomUUID(),
    publicId: randomUUID(),
    userId: randomUUID(),
...
describe("ProcessEscrowEventUseCase", () => {
  ...
  describe("EscrowCreated event", () => {
    it("updates a PROCESSING escrow to ON_CHAIN and sets onChainEscrowId", async () => {
      const escrow = makeEscrow("PROCESSING" /* PROCESSING */, TX_HASH);
      ...
      globalExpect(updated.onChainEscrowId).toBe(ESCROW_ID);
    });
    it("does not update an escrow that is not in PROCESSING status", async () => {
      const escrow = makeEscrow("PENDING" /* PENDING */, TX_HASH);
      await escrowRepo.save(escrow);
      await useCase.execute([
        { tx_hash: TX_HASH, escrow_id: ESCROW_ID, event_type: "EscrowCreated", block_number: "100" }
      ]);
      const notUpdated = await escrowRepo.findByTxHash(TX_HASH);
      globalExpect(notUpdated.status).toBe("PENDING" /* PENDING */);
    });
    it("buffers the event when no matching escrow is found", async () => {
      ...
      globalExpect(buffered.escrowId).toBe(ESCROW_ID);
      globalExpect(buffered.eventType).toBe("EscrowCreated");
    });
    it("buffers event when escrow exists but is not PROCESSING", async () => {
      const escrow = makeEscrow("ON_CHAIN" /* ON_CHAIN */, TX_HASH);
      await escrowRepo.save(escrow);
      await useCase.execute([
        { tx_hash: TX_HASH, escrow_id: ESCROW_ID, event_type: "EscrowCreated", block_number: "100" }
      ]);
  ...
  describe("EscrowSettled event", () => {
    it("updates an ON_CHAIN escrow to SETTLED", async () => {
      ...
      globalExpect(updated.status).toBe("SETTLED" /* SETTLED */);
    });
    ...
    it("does not update escrow that is not in ON_CHAIN status", async () => {
      const escrow = makeEscrow("SETTLED" /* SETTLED */, void 0, ESCROW_ID);
      await escrowRepo.save(escrow);
      await useCase.execute([
        { tx_hash: TX_HASH, escrow_id: ESCROW_ID, event_type: "EscrowSettled", block_number: "200" }
      ]);
      const unchanged = await escrowRepo.findByOnChainId(ESCROW_ID);
      globalExpect(unchanged.status).toBe("SETTLED" /* SETTLED */);
    });
  });
  it("processes multiple events in sequence", async () => {
    const esc1 = makeEscrow("PROCESSING" /* PROCESSING */, "0xtx1");
    const esc2 = makeEscrow("ON_CHAIN" /* ON_CHAIN */, void 0, "esc-2");
    ...
    globalExpect(updated1.status).toBe("ON_CHAIN" /* ON_CHAIN */);
    const updated2 = await escrowRepo.findByOnChainId("esc-2");
    globalExpect(updated2.status).toBe("SETTLED" /* SETTLED */);
  });
});
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=process-escrow-event.use-case.test.js.map