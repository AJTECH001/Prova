...
// src/application/use-case/credential/__tests__/generate-credentials.use-case.test.ts
...
describe("GenerateCredentialsUseCase", () => {
  ...
  it("returns client_id starting with rc_", async () => {
    ...
    globalExpect(result.client_id).toMatch(/^rc_/);
  });
  it("returns a client_secret as a hex string", async () => {
    const result = await useCase.execute(USER_ID);
    globalExpect(result.client_secret).toMatch(/^[0-9a-f]+$/);
    globalExpect(result.client_secret.length).toBeGreaterThan(0);
  });
  it("returns status active and a created_at timestamp", async () => {
    const result = await useCase.execute(USER_ID);
    globalExpect(result.status).toBe("active");
    globalExpect(typeof result.created_at).toBe("string");
    globalExpect(() => new Date(result.created_at)).not.toThrow();
  });
  it("saves credential to repository with hashed secret (not plaintext)", async () => {
    ...
    globalExpect(saved.hashedSecret).not.toBe(result.client_secret);
    globalExpect(saved.salt).toBeDefined();
  });
  it("generates unique client_id on each call", async () => {
    ...
    globalExpect(r1.client_id).not.toBe(r2.client_id);
  });
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=generate-credentials.use-case.test.js.map