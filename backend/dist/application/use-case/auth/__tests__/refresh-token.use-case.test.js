...
// src/application/use-case/auth/__tests__/refresh-token.use-case.test.ts
...
describe("RefreshTokenUseCase", () => {
  ...
  it("rotates the session \u2014 deletes old session and creates a new one", async () => {
    ...
    globalExpect(oldSessionGone).toBeNull();
    const newSession = await sessionRepo.findByRefreshToken(firstResult.refresh_token);
    globalExpect(newSession).not.toBeNull();
    globalExpect(newSession.id).not.toBe(oldSessionId);
  });
  ...
  it("throws unauthorized for a completely invalid token", async () => {
    await globalExpect(useCase.execute({ refresh_token: "not-a-jwt" })).rejects.toMatchObject({
      statusCode: 401
    });
  });
  it("throws unauthorized when session is expired", async () => {
    ...
    await globalExpect(useCase.execute({ refresh_token: tokenPair.refreshToken })).rejects.toMatchObject({
      statusCode: 401
    });
  });
});
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=refresh-token.use-case.test.js.map