...
// src/application/use-case/auth/__tests__/verify-wallet.use-case.test.ts
var WALLET = "0xabCDEF1234567890ABcDEF1234567890aBCDeF12";
...
describe("VerifyWalletUseCase", () => {
  ...
  it("creates a new user and returns token pair when wallet is not registered", async () => {
    ...
    globalExpect(result).toHaveProperty("access_token");
    globalExpect(result).toHaveProperty("refresh_token");
    globalExpect(result.token_type).toBe("Bearer");
    globalExpect(result.expires_in).toBeGreaterThan(0);
    const user = await userRepo.findByWalletAddress(WALLET);
    globalExpect(user).not.toBeNull();
  });
  it("finds existing user instead of creating a new one", async () => {
    ...
    globalExpect(users).not.toBeNull();
    const sessions = await sessionRepo.findByUserId(users.id);
    globalExpect(sessions.length).toBeGreaterThanOrEqual(1);
  });
  it("creates a session and stores the refresh token", async () => {
    ...
    globalExpect(session).not.toBeNull();
  });
  it("throws unauthorized when SIWE signature is invalid", async () => {
    ...
    await globalExpect(useCase.execute({ wallet_address: WALLET, message: "msg", signature: "0xbad" })).rejects.toThrow(
      ApplicationHttpError
    );
  });
  it("throws unauthorized when nonce is invalid", async () => {
    const message = buildSiweMessage("wrongnonce");
    await globalExpect(useCase.execute({ wallet_address: WALLET, message, signature: "0xsig" })).rejects.toMatchObject({
      statusCode: 401
    });
  });
});
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=verify-wallet.use-case.test.js.map