...
// src/application/use-case/auth/__tests__/request-nonce.use-case.test.ts
describe("RequestNonceUseCase", () => {
  ...
  it("returns a nonce for the given wallet address", async () => {
    ...
    globalExpect(result).toHaveProperty("nonce");
    globalExpect(typeof result.nonce).toBe("string");
    globalExpect(result.nonce.length).toBeGreaterThan(0);
  });
  it("returns a hex string of 64 characters", async () => {
    const result = await useCase.execute({ wallet_address: "0xabc" });
    globalExpect(result.nonce).toMatch(/^[0-9a-f]{64}$/);
  });
  it("generates different nonces for sequential calls", async () => {
    ...
    globalExpect(r1.nonce).not.toBe(r2.nonce);
  });
});
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=request-nonce.use-case.test.js.map