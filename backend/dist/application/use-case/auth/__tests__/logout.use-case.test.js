...
// src/application/use-case/auth/__tests__/logout.use-case.test.ts
...
describe("LogoutUseCase", () => {
  ...
  it("deletes all sessions for the given user", async () => {
    ...
    globalExpect(remaining).toHaveLength(0);
  });
  it("does not affect sessions of other users", async () => {
    ...
    globalExpect(others).toHaveLength(1);
  });
  it("resolves without error when user has no sessions", async () => {
    await globalExpect(useCase.execute(randomUUID())).resolves.toBeUndefined();
  });
});
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=logout.use-case.test.js.map