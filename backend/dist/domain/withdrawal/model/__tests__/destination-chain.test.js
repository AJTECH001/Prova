...
// src/domain/withdrawal/model/destination-chain.enum.ts
var DestinationChain = /* @__PURE__ */ ((DestinationChain2) => {
  DestinationChain2[DestinationChain2["ETH"] = 0] = "ETH";
  DestinationChain2[DestinationChain2["BASE"] = 6] = "BASE";
  DestinationChain2[DestinationChain2["POLYGON"] = 7] = "POLYGON";
  return DestinationChain2;
})(DestinationChain || {});

// src/domain/withdrawal/model/__tests__/destination-chain.test.ts
describe("DestinationChain", () => {
  it("ETH has value 0", () => {
    globalExpect(0 /* ETH */).toBe(0);
  });
  it("BASE has value 6", () => {
    globalExpect(6 /* BASE */).toBe(6);
  });
  it("POLYGON has value 7", () => {
    globalExpect(7 /* POLYGON */).toBe(7);
  });
  it("has exactly three members", () => {
    const numericKeys = Object.keys(DestinationChain).filter((k2) => !isNaN(Number(k2)));
    globalExpect(numericKeys).toHaveLength(3);
  });
  it("numeric keys map back to enum names", () => {
    globalExpect(DestinationChain[0]).toBe("ETH");
    globalExpect(DestinationChain[6]).toBe("BASE");
    globalExpect(DestinationChain[7]).toBe("POLYGON");
  });
});
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=destination-chain.test.js.map