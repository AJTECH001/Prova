...
// src/domain/withdrawal/model/__tests__/withdrawal.test.ts
function makeWithdrawalParams(overrides) {
  return {
    id: "wd-1",
    publicId: "pub-wd-1",
    userId: "user-1",
...
describe("Withdrawal", () => {
  describe("constructor", () => {
    it("assigns all required fields", () => {
      ...
      globalExpect(withdrawal.id).toBe(params.id);
      globalExpect(withdrawal.publicId).toBe(params.publicId);
      globalExpect(withdrawal.userId).toBe(params.userId);
      globalExpect(withdrawal.walletId).toBe(params.walletId);
      globalExpect(withdrawal.escrowIds).toEqual(params.escrowIds);
      globalExpect(withdrawal.destinationChain).toBe(params.destinationChain);
      globalExpect(withdrawal.destinationDomain).toBe(params.destinationDomain);
      globalExpect(withdrawal.recipientAddress).toBe(params.recipientAddress);
      globalExpect(withdrawal.status).toBe(params.status);
      globalExpect(withdrawal.estimatedAmount).toBe(params.estimatedAmount);
      globalExpect(withdrawal.walletProvider).toBe(params.walletProvider);
      globalExpect(withdrawal.createdAt).toBe(params.createdAt);
      globalExpect(withdrawal.updatedAt).toBe(params.updatedAt);
    });
    it("assigns optional fields when provided", () => {
      const params = makeWithdrawalParams({
        ...
        completedAt: /* @__PURE__ */ new Date("2025-02-01")
      });
      const withdrawal = new Withdrawal(params);
      globalExpect(withdrawal.actualAmount).toBe(495);
      globalExpect(withdrawal.fee).toBe(5);
      globalExpect(withdrawal.redeemTxHash).toBe(params.redeemTxHash);
      globalExpect(withdrawal.bridgeTxHash).toBe(params.bridgeTxHash);
      globalExpect(withdrawal.destinationTxHash).toBe(params.destinationTxHash);
      globalExpect(withdrawal.errorMessage).toBe("some error");
      globalExpect(withdrawal.completedAt).toBe(params.completedAt);
    });
    it("leaves optional fields undefined when not provided", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams());
      globalExpect(withdrawal.actualAmount).toBeUndefined();
      globalExpect(withdrawal.fee).toBeUndefined();
      globalExpect(withdrawal.redeemTxHash).toBeUndefined();
      globalExpect(withdrawal.bridgeTxHash).toBeUndefined();
      globalExpect(withdrawal.destinationTxHash).toBeUndefined();
      globalExpect(withdrawal.errorMessage).toBeUndefined();
      globalExpect(withdrawal.completedAt).toBeUndefined();
    });
  });
  describe("markRedeemComplete", () => {
    it("sets redeemTxHash", () => {
      ...
      globalExpect(withdrawal.redeemTxHash).toBe(hash);
    });
    it("changes status to PENDING_BRIDGE", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams());
      withdrawal.markRedeemComplete("0x" + "a".repeat(64));
      globalExpect(withdrawal.status).toBe("PENDING_BRIDGE" /* PENDING_BRIDGE */);
    });
    it("updates updatedAt", () => {
      const originalUpdatedAt = /* @__PURE__ */ new Date("2025-01-01");
      const withdrawal = new Withdrawal(makeWithdrawalParams({ updatedAt: originalUpdatedAt }));
      withdrawal.markRedeemComplete("0x" + "a".repeat(64));
      globalExpect(withdrawal.updatedAt).not.toBe(originalUpdatedAt);
    });
    it("returns this for chaining", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams());
      globalExpect(withdrawal.markRedeemComplete("0x" + "a".repeat(64))).toBe(withdrawal);
    });
  });
  describe("markBridgeInitiated", () => {
    it("sets bridgeTxHash", () => {
      const hash = "0x" + "b".repeat(64);
      const withdrawal = new Withdrawal(makeWithdrawalParams({ status: "PENDING_BRIDGE" /* PENDING_BRIDGE */ }));
      withdrawal.markBridgeInitiated(hash);
      globalExpect(withdrawal.bridgeTxHash).toBe(hash);
    });
    it("changes status to BRIDGING", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams({ status: "PENDING_BRIDGE" /* PENDING_BRIDGE */ }));
      withdrawal.markBridgeInitiated("0x" + "b".repeat(64));
      globalExpect(withdrawal.status).toBe("BRIDGING" /* BRIDGING */);
    });
    it("updates updatedAt", () => {
      const originalUpdatedAt = /* @__PURE__ */ new Date("2025-01-01");
      const withdrawal = new Withdrawal(
        makeWithdrawalParams({ status: "PENDING_BRIDGE" /* PENDING_BRIDGE */, updatedAt: originalUpdatedAt })
      );
      withdrawal.markBridgeInitiated("0x" + "b".repeat(64));
      globalExpect(withdrawal.updatedAt).not.toBe(originalUpdatedAt);
    });
    it("returns this for chaining", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams({ status: "PENDING_BRIDGE" /* PENDING_BRIDGE */ }));
      globalExpect(withdrawal.markBridgeInitiated("0x" + "b".repeat(64))).toBe(withdrawal);
    });
  });
  describe("markCompleted", () => {
    it("sets destinationTxHash", () => {
      const hash = "0x" + "c".repeat(64);
      const withdrawal = new Withdrawal(makeWithdrawalParams({ status: "BRIDGING" /* BRIDGING */ }));
      withdrawal.markCompleted(hash);
      globalExpect(withdrawal.destinationTxHash).toBe(hash);
    });
    it("changes status to COMPLETED", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams({ status: "BRIDGING" /* BRIDGING */ }));
      withdrawal.markCompleted("0x" + "c".repeat(64));
      globalExpect(withdrawal.status).toBe("COMPLETED" /* COMPLETED */);
    });
    it("sets completedAt", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams({ status: "BRIDGING" /* BRIDGING */ }));
      globalExpect(withdrawal.completedAt).toBeUndefined();
      withdrawal.markCompleted("0x" + "c".repeat(64));
      globalExpect(withdrawal.completedAt).toBeInstanceOf(Date);
    });
    it("updates updatedAt", () => {
      const originalUpdatedAt = /* @__PURE__ */ new Date("2025-01-01");
      const withdrawal = new Withdrawal(
        makeWithdrawalParams({ status: "BRIDGING" /* BRIDGING */, updatedAt: originalUpdatedAt })
      );
      withdrawal.markCompleted("0x" + "c".repeat(64));
      globalExpect(withdrawal.updatedAt).not.toBe(originalUpdatedAt);
    });
    it("returns this for chaining", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams({ status: "BRIDGING" /* BRIDGING */ }));
      globalExpect(withdrawal.markCompleted("0x" + "c".repeat(64))).toBe(withdrawal);
    });
  });
  describe("markFailed", () => {
    it("sets errorMessage", () => {
      ...
      globalExpect(withdrawal.errorMessage).toBe("bridge timeout");
    });
    it("changes status to FAILED", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams());
      withdrawal.markFailed("bridge timeout");
      globalExpect(withdrawal.status).toBe("FAILED" /* FAILED */);
    });
    it("updates updatedAt", () => {
      const originalUpdatedAt = /* @__PURE__ */ new Date("2025-01-01");
      const withdrawal = new Withdrawal(makeWithdrawalParams({ updatedAt: originalUpdatedAt }));
    ...
    it("returns this for chaining", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams());
      globalExpect(withdrawal.markFailed("error")).toBe(withdrawal);
    });
  });
  describe("canCreateBridgeChallenge", () => {
    it("returns true when status is PENDING_BRIDGE", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams({ status: "PENDING_BRIDGE" /* PENDING_BRIDGE */ }));
      globalExpect(withdrawal.canCreateBridgeChallenge()).toBe(true);
    });
    it("returns false when status is PENDING_REDEEM", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams({ status: "PENDING_REDEEM" /* PENDING_REDEEM */ }));
      globalExpect(withdrawal.canCreateBridgeChallenge()).toBe(false);
    });
    ...
    it("returns false when status is COMPLETED", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams({ status: "COMPLETED" /* COMPLETED */ }));
      globalExpect(withdrawal.canCreateBridgeChallenge()).toBe(false);
    });
    it("returns false when status is FAILED", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams({ status: "FAILED" /* FAILED */ }));
      globalExpect(withdrawal.canCreateBridgeChallenge()).toBe(false);
    });
  });
  describe("isTerminal", () => {
    it("returns true when status is COMPLETED", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams({ status: "COMPLETED" /* COMPLETED */ }));
      globalExpect(withdrawal.isTerminal()).toBe(true);
    });
    ...
    it("returns false when status is PENDING_REDEEM", () => {
      const withdrawal = new Withdrawal(makeWithdrawalParams({ status: "PENDING_REDEEM" /* PENDING_REDEEM */ }));
      globalExpect(withdrawal.isTerminal()).toBe(false);
    });
