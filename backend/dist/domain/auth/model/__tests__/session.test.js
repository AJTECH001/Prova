...
// src/domain/auth/model/__tests__/session.test.ts
...
describe("Session", () => {
  describe("constructor", () => {
    it("assigns all required fields", () => {
      ...
      globalExpect(session.id).toBe(params.id);
      globalExpect(session.userId).toBe(params.userId);
      globalExpect(session.refreshToken).toBe(params.refreshToken);
      globalExpect(session.expiresAt).toBe(params.expiresAt);
      globalExpect(session.createdAt).toBe(params.createdAt);
    });
    it("assigns optional userAgent when provided", () => {
      const session = new Session(makeSessionParams({ userAgent: "Mozilla/5.0" }));
      globalExpect(session.userAgent).toBe("Mozilla/5.0");
    });
    it("assigns optional ipAddress when provided", () => {
      const session = new Session(makeSessionParams({ ipAddress: "192.168.1.1" }));
      globalExpect(session.ipAddress).toBe("192.168.1.1");
    });
    it("leaves optional fields undefined when not provided", () => {
      const session = new Session(makeSessionParams());
      globalExpect(session.userAgent).toBeUndefined();
      globalExpect(session.ipAddress).toBeUndefined();
    });
  });
  describe("isExpired", () => {
    it("returns false when expiresAt is in the future", () => {
      const session = new Session(makeSessionParams({ expiresAt: new Date(Date.now() + 1e4) }));
      globalExpect(session.isExpired()).toBe(false);
    });
    it("returns true when expiresAt is in the past", () => {
      const session = new Session(makeSessionParams({ expiresAt: new Date(Date.now() - 1e3) }));
      globalExpect(session.isExpired()).toBe(true);
    });
  ...
  describe("getTtlSeconds", () => {
    it("returns approximate seconds remaining for a future session", () => {
      ...
      globalExpect(ttl).toBeGreaterThan(0);
      globalExpect(ttl).toBeLessThanOrEqual(30);
    });
    it("returns 0 when session is already expired", () => {
      const session = new Session(makeSessionParams({ expiresAt: new Date(Date.now() - 5e3) }));
      globalExpect(session.getTtlSeconds()).toBe(0);
    });
    ...
    it("never returns a negative value", () => {
      const session = new Session(makeSessionParams({ expiresAt: new Date(Date.now() - 1e5) }));
      globalExpect(session.getTtlSeconds()).toBeGreaterThanOrEqual(0);
    });
  });
});
