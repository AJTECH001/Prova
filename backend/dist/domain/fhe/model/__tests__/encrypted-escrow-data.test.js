...
// src/domain/fhe/model/__tests__/encrypted-escrow-data.test.ts
function makeEncryptedValue(userAddress, data = "0xdata") {
...
function makeEncryptedEscrowDataParams(overrides) {
...
describe("EncryptedEscrowData", () => {
  describe("constructor", () => {
    it("assigns all required fields", () => {
      ...
      globalExpect(eid.encryptedAmount).toBe(params.encryptedAmount);
      globalExpect(eid.encryptedOwner).toBe(params.encryptedOwner);
      globalExpect(eid.userAddress).toBe(params.userAddress);
    });
    it("assigns optional plaintextAmount when provided", () => {
      const eid = new EncryptedEscrowData(makeEncryptedEscrowDataParams({ plaintextAmount: BigInt(1e3) }));
      globalExpect(eid.plaintextAmount).toBe(BigInt(1e3));
    });
    it("assigns optional plaintextOwner when provided", () => {
      ...
      globalExpect(eid.plaintextOwner).toBe("0xowner");
    });
    it("leaves optional fields undefined when not provided", () => {
      const eid = new EncryptedEscrowData(makeEncryptedEscrowDataParams());
      globalExpect(eid.plaintextAmount).toBeUndefined();
      globalExpect(eid.plaintextOwner).toBeUndefined();
    });
  });
  describe("isForUser", () => {
    it("returns true when address matches exactly", () => {
      ...
      globalExpect(eid.isForUser(address)).toBe(true);
    });
    it("returns true with case-insensitive match", () => {
      ...
      globalExpect(eid.isForUser("0xABCDEF1234567890ABCDEF1234567890ABCDEF12")).toBe(true);
    });
    it("returns false when address does not match", () => {
      ...
      globalExpect(eid.isForUser("0x1111111111111111111111111111111111111111")).toBe(false);
    });
  });
  describe("getContractCallParameters", () => {
    it("returns encrypted_owner tuple from encryptedOwner", () => {
      ...
      globalExpect(result.encrypted_owner).toEqual(params.encryptedOwner.toTuple());
    });
    it("returns encrypted_amount tuple from encryptedAmount", () => {
      const params = makeEncryptedEscrowDataParams();
      const eid = new EncryptedEscrowData(params);
      const result = eid.getContractCallParameters();
      globalExpect(result.encrypted_amount).toEqual(params.encryptedAmount.toTuple());
    });
    it("returns zero address as resolver", () => {
      const eid = new EncryptedEscrowData(makeEncryptedEscrowDataParams());
      const result = eid.getContractCallParameters();
      globalExpect(result.resolver).toBe("0x0000000000000000000000000000000000000000");
    });
    it("returns 0x as resolver_data", () => {
      const eid = new EncryptedEscrowData(makeEncryptedEscrowDataParams());
      const result = eid.getContractCallParameters();
      globalExpect(result.resolver_data).toBe("0x");
    });
  });
  describe("toJSON", () => {
    it("serializes all fields to a plain object", () => {
      ...
      globalExpect(json.userAddress).toBe(params.userAddress);
      globalExpect(json.encryptedAmount).toEqual(params.encryptedAmount.toJSON());
      globalExpect(json.encryptedOwner).toEqual(params.encryptedOwner.toJSON());
      globalExpect(json.plaintextAmount).toBe("500");
      globalExpect(json.plaintextOwner).toBe("0xowner");
    });
    it("omits plaintextAmount in JSON when undefined", () => {
      const eid = new EncryptedEscrowData(makeEncryptedEscrowDataParams());
      const json = eid.toJSON();
      globalExpect(json.plaintextAmount).toBeUndefined();
    });
    it("omits plaintextOwner in JSON when undefined", () => {
      const eid = new EncryptedEscrowData(makeEncryptedEscrowDataParams());
      const json = eid.toJSON();
      globalExpect(json.plaintextOwner).toBeUndefined();
    });
  });
  describe("fromJSON", () => {
    it("restores an EncryptedEscrowData from JSON", () => {
      ...
      globalExpect(restored).toBeInstanceOf(EncryptedEscrowData);
      globalExpect(restored.userAddress).toBe(params.userAddress);
      globalExpect(restored.plaintextAmount).toBe(BigInt(999));
    });
    it("restores plaintextOwner from JSON", () => {
      const params = makeEncryptedEscrowDataParams({ plaintextOwner: "0xsome-owner" });
      const original = new EncryptedEscrowData(params);
      const restored = EncryptedEscrowData.fromJSON(original.toJSON());
      globalExpect(restored.plaintextOwner).toBe("0xsome-owner");
    });
    it("leaves plaintextAmount undefined when not present in JSON", () => {
      ...
      const restored = EncryptedEscrowData.fromJSON(original.toJSON());
      globalExpect(restored.plaintextAmount).toBeUndefined();
    });
    it("round-trips through toJSON and fromJSON", () => {
      const params = makeEncryptedEscrowDataParams({ plaintextAmount: BigInt(42) });
      const original = new EncryptedEscrowData(params);
      const restored = EncryptedEscrowData.fromJSON(original.toJSON());
      globalExpect(restored.toJSON()).toEqual(original.toJSON());
    });
    it("restores nested EncryptedValue instances", () => {
      const original = new EncryptedEscrowData(makeEncryptedEscrowDataParams());
      const restored = EncryptedEscrowData.fromJSON(original.toJSON());
      globalExpect(restored.encryptedAmount).toBeInstanceOf(EncryptedValue);
      globalExpect(restored.encryptedOwner).toBeInstanceOf(EncryptedValue);
    });
  });
});
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=encrypted-escrow-data.test.js.map