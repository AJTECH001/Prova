...
// src/domain/fhe/model/__tests__/encrypted-value.test.ts
function makeEncryptedValueParams(overrides) {
  return {
    type: "euint64",
    data: "0xdeadbeef",
    securityZone: 0,
...
describe("EncryptedValue", () => {
  describe("constructor", () => {
    it("assigns all fields", () => {
      ...
      globalExpect(ev.type).toBe(params.type);
      globalExpect(ev.data).toBe(params.data);
      globalExpect(ev.securityZone).toBe(params.securityZone);
      globalExpect(ev.utype).toBe(params.utype);
      globalExpect(ev.inputProof).toBe(params.inputProof);
      globalExpect(ev.userAddress).toBe(params.userAddress);
    });
    it("accepts all valid EncryptedValueType variants", () => {
      ...
      globalExpect(euint64.type).toBe("euint64");
      globalExpect(eaddress.type).toBe("eaddress");
      globalExpect(ebool.type).toBe("ebool");
    });
  });
  describe("isForUser", () => {
    it("returns true when address matches exactly", () => {
      ...
      globalExpect(ev.isForUser(address)).toBe(true);
    });
    it("returns true with case-insensitive match \u2014 stored lowercase, queried uppercase", () => {
      ...
      globalExpect(ev.isForUser("0xABCDEF1234567890ABCDEF1234567890ABCDEF12")).toBe(true);
    });
    ...
    it("returns false when address does not match", () => {
      ...
      globalExpect(ev.isForUser("0x1111111111111111111111111111111111111111")).toBe(false);
    });
  });
  describe("toTuple", () => {
    it("returns [data, securityZone, utype, inputProof]", () => {
      const params = makeEncryptedValueParams();
      const ev = new EncryptedValue(params);
      globalExpect(ev.toTuple()).toEqual([params.data, params.securityZone, params.utype, params.inputProof]);
    });
    it("returns an array with exactly 4 elements", () => {
      const ev = new EncryptedValue(makeEncryptedValueParams());
      globalExpect(ev.toTuple()).toHaveLength(4);
    });
  });
  describe("toJSON", () => {
  ...
  describe("fromJSON", () => {
    it("constructs an EncryptedValue from a JSON object", () => {
      ...
      globalExpect(restored.type).toBe(params.type);
      globalExpect(restored.data).toBe(params.data);
      globalExpect(restored.securityZone).toBe(params.securityZone);
      globalExpect(restored.utype).toBe(params.utype);
      globalExpect(restored.inputProof).toBe(params.inputProof);
    ...
    it("returns an instance of EncryptedValue", () => {
      const original = new EncryptedValue(makeEncryptedValueParams());
      const restored = EncryptedValue.fromJSON(original.toJSON());
      globalExpect(restored).toBeInstanceOf(EncryptedValue);
    });
  });
});
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=encrypted-value.test.js.map