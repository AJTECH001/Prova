...
// src/domain/api-credential/model/__tests__/api-credential.test.ts
function makeApiCredentialParams(overrides) {
  return {
    id: "cred-1",
    clientId: "client-abc",
    userId: "user-1",
...
describe("ApiCredential", () => {
  describe("constructor", () => {
    it("assigns all required fields", () => {
      ...
      globalExpect(cred.id).toBe(params.id);
      globalExpect(cred.clientId).toBe(params.clientId);
      globalExpect(cred.userId).toBe(params.userId);
      globalExpect(cred.hashedSecret).toBe(params.hashedSecret);
      globalExpect(cred.salt).toBe(params.salt);
      globalExpect(cred.status).toBe(params.status);
      globalExpect(cred.createdAt).toBe(params.createdAt);
    });
    it("assigns optional lastUsedAt when provided", () => {
      const lastUsedAt = /* @__PURE__ */ new Date("2025-06-01");
      const cred = new ApiCredential(makeApiCredentialParams({ lastUsedAt }));
      globalExpect(cred.lastUsedAt).toBe(lastUsedAt);
    });
    it("leaves lastUsedAt undefined when not provided", () => {
      const cred = new ApiCredential(makeApiCredentialParams());
      globalExpect(cred.lastUsedAt).toBeUndefined();
    });
  });
  describe("revoke", () => {
    it("changes status to revoked", () => {
      ...
      cred.revoke();
      globalExpect(cred.status).toBe("revoked");
    });
    it("returns this for chaining", () => {
      const cred = new ApiCredential(makeApiCredentialParams());
      globalExpect(cred.revoke()).toBe(cred);
    });
  ...
  describe("touch", () => {
    it("sets lastUsedAt to a recent date", () => {
      ...
      globalExpect(cred.lastUsedAt).toBeInstanceOf(Date);
      globalExpect(cred.lastUsedAt.getTime()).toBeGreaterThanOrEqual(before.getTime());
      globalExpect(cred.lastUsedAt.getTime()).toBeLessThanOrEqual(after.getTime());
    });
    it("overwrites an existing lastUsedAt", () => {
      const oldDate = /* @__PURE__ */ new Date("2020-01-01");
      const cred = new ApiCredential(makeApiCredentialParams({ lastUsedAt: oldDate }));
      cred.touch();
      globalExpect(cred.lastUsedAt).not.toBe(oldDate);
    });
    it("returns this for chaining", () => {
      const cred = new ApiCredential(makeApiCredentialParams());
      globalExpect(cred.touch()).toBe(cred);
    });
  });
  describe("isActive", () => {
    it("returns true when status is active", () => {
      const cred = new ApiCredential(makeApiCredentialParams({ status: "active" }));
      globalExpect(cred.isActive()).toBe(true);
    });
    it("returns false when status is revoked", () => {
      const cred = new ApiCredential(makeApiCredentialParams({ status: "revoked" }));
      globalExpect(cred.isActive()).toBe(false);
    });
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=api-credential.test.js.map