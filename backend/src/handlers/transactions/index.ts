...
import { CreateEscrowDtoSchema } from '../../application/dto/escrow/create-escrow.dto.js';
import {
  CreateEscrowUseCase,
  type EncryptionMode,
} from '../../application/use-case/escrow/create-escrow.use-case.js';
import { GetEscrowsUseCase } from '../../application/use-case/escrow/get-escrows.use-case.js';
...
const getHandler = createGetHandler({
  ...
  execute: async (req, authPayload) => {
    ...
    const result = await getEscrowsUseCase.execute(authPayload!.userId, { limit, cursor, status });
