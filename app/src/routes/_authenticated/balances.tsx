...
function StatCard({ label, value, sub, loading }: { label: string; value: string; sub?: string; loading?: boolean }) {
  return (
    <div className="rounded-xl border border-[var(--border-dark)] bg-white px-5 py-5 shadow-[var(--shadow-card)]">
      <p className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)]">{label}</p>
      {loading ? (
        <Skeleton className="mt-2 h-7 w-28" />
      ) : (
        <p className="mt-1.5 text-2xl font-bold tabular-nums text-[var(--text-primary)]">{value}</p>
      )}
...
function ChevronRight() {
  return (
    <svg width="16" height="16" viewBox="0 0 20 20" fill="currentColor" className="shrink-0 text-[var(--text-muted)]">
...
export function BalancesPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-xl font-bold tracking-tight text-[var(--text-primary)] sm:text-2xl">Balances</h1>
        <p className="mt-0.5 text-sm text-[var(--text-muted)]">Your account balance and fund activity</p>
      </div>

      {/* Primary balance card */}
      <div className="rounded-xl border border-[var(--border-dark)] bg-white p-6 shadow-[var(--shadow-card)]">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            {balanceLoading && !balance ? (
              ...
              <p className="mt-1 text-4xl font-bold tabular-nums tracking-tight text-[var(--text-primary)]">
                {balance ? balance.formatted_balance : '—'}
                <span className="ml-2 text-base font-normal text-[var(--text-muted)]">{balance?.currency ?? 'USDC'}</span>
              </p>
            )}
            <p className="mt-2 text-xs text-[var(--text-muted)]">Funds available to use or withdraw to your bank</p>
          </div>
      ...
      <div className="rounded-xl border border-[var(--border-dark)] bg-white shadow-[var(--shadow-card)]">
        ...
        <div className="flex flex-col divide-y divide-[var(--border-dark)]">
          <Link
            href="/transactions"
            className="flex items-center justify-between px-5 py-4 transition-colors hover:bg-[hsl(var(--bg-hover))]"
          >
            <div>
              <p className="text-sm font-medium text-[var(--text-primary)]">View transactions</p>
              <p className="text-xs text-[var(--text-muted)]">See all your payments and their current status</p>
            </div>
            <ChevronRight />
          </Link>
          <Link
          ...
          >
            <div>
              <p className="text-sm font-medium text-[var(--text-primary)]">Withdraw funds</p>
              <p className="text-xs text-[var(--text-muted)]">Transfer settled funds to your wallet</p>
            </div>
            <ChevronRight />
          </Link>
          <Link
          ...
          >
            <div>
              <p className="text-sm font-medium text-[var(--text-primary)]">Disputes & claims</p>
              <p className="text-xs text-[var(--text-muted)]">Review overdue payments and file insurance claims</p>
            </div>
            <ChevronRight />
          </Link>
        </div>
